  ...
  return (
    <section className="mb-12">
      <div className="flex justify-between items-center mb-6">
        ...
        <a href="#" className="text-[#0a7a35] font-medium hover:underline">View all &gt;</a>
      </div>
        ...
        {products.map((product) => (
          <div key={product._id || product.id} className="border border-gray-100 rounded-xl overflow-hidden hover:shadow-lg transition duration-300 group">
            ...
            <div className="relative h-52 overflow-hidden">
              ...
              />
              <button className="absolute bottom-3 right-3 bg-[#0a7a35] hover:bg-green-800 text-white rounded-full w-9 h-9 flex items-center justify-center text-xl transition shadow-md">
                +
              </button>
            </div>
            ...
            <div className="p-4">
              ...
              <p className="text-lg font-bold text-[#0a7a35]">{typeof product.price === 'number' ? `$${product.price.toFixed(2)}` : product.price}</p>
            </div>
